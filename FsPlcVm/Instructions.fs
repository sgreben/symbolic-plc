...
module Instructions =
    ...
    module Nullop =
        let inline _not s = 
            Vmp.set_cr_value_basic (Vmp.cr_value_basic s |> Vms.complement) s
    ...
    module Storeop =
        ...
        let inline _store_index (_array:Array_repr_boxed) i v s = 
            Vmp.write _array.[Semantics.int_value i] v s
        ...
        let inline s_index_ref r i s = _store_index (Vmp.read_boxed_array r s) i (Vmp.cr_value |> Semantics.is_boolean_one) s
        ...
        let inline r_index_ref r i s = _store_index (Vmp.read_boxed_array r s) i (Vmp.cr_value |> Semantics.is_boolean_zero) s
        let inline step_ref op r s =
            match op with
            | ST -> store_ref r s
            | STN -> store_refn r s
    ...
    module Funop =
        ...
        module Builtin =
            module Extop =
                let inline add vs s = vs |> List.fold Vms.add (Vmp.cr_value_basic s) |> fun v -> Vmp.set_cr_value_basic v s
                let inline values vs s = vs |> List.map (function 
                    | Value (VBasic v) -> v
    ...
    module Osop =
        ...
        let inline begin_task i tid s = 
            Vmp.set_task_current (i,tid) s |> Vmp.inc_task_count tid
    ...
    module Monitorop =
        ...
        let inline log_state s = printfn "<<<MONITOR>>>%A" s; s
